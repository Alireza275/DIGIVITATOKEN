var CONTRACT_ADDRESS = "0x0000000000000000000000000000000000";
...
var referrer = '0x00000000000000000000000000000000000000000'
var upline = '0x00000000000000000000000000000000000000000'
    ...
    setInterval(() => {
        if(contract){
            ...
            contract.methods.totalTokenStaked().call().then(res=>{                	
				$("#total-token-staked").html(`Total Staked<span>${(res/1e18).toFixed(2)} NETWORK</span>`)
            })	
			
		
			// todo, change hardcoded address for variable
			//tokenContract.methods.balanceOf('0x0000000000000000000000000000000000000').call().then(res => {
           ...
           contract.methods.getUserBonAirdrop(currentAddr).call().then(res=>{                	
            ...
            contract.methods.getUserTokenStaked(currentAddr).call().then(res=>{                	
				$("#user-token-staked").html(`My Stake<span>${(res/1e18).toFixed(2)} DIGIVITA</span>`)
            })				
...
function stakeM() {
	...
	if (CurrentDateTime > BUSDLaunchDateTime) {	// launched
		...
		window.alert("Staking BUSD available on January 25 2021 14:00:00 GMT-0000");
	}
}
